...
// Step 1: sort positions
    ...
    while (i < x_count) {
        ...
        // Step 2: move to the farthest house within k from x[i]
        int loc = x[i] + k;
        while (i < x_count && x[i] <= loc) {
            i++;
        }

        // Step 3: place transmitter at farthest house to left
        int transmitter = x[i - 1] + k;
        while (i < x_count && x[i] <= transmitter) {
            i++;
        }
    }
    return count;
