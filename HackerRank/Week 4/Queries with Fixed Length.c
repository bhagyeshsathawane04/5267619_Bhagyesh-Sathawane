 ...
 int *answers = (int*)malloc(queries_count * sizeof(int));
    *result_count = queries_count;
    ...
    for (int qi = 0; qi < queries_count; qi++) {
        int d = queries[qi];
        Deque dq;
        initDeque(&dq, arr_count);
        int min_of_max = 1000001; // arr[i] < 1e6

        // Process first d elements
        for (int i = 0; i < d; i++) {
            while (!empty(&dq) && arr[back(&dq)] <= arr[i]) pop_back(&dq);
            push_back(&dq, i);
        }
        min_of_max = arr[front(&dq)];

        // Process remaining elements
        for (int i = d; i < arr_count; i++) {
            // remove elements outside window
            if (front(&dq) <= i - d) pop_front(&dq);
            // add new element
            while (!empty(&dq) && arr[back(&dq)] <= arr[i]) pop_back(&dq);
            push_back(&dq, i);
            if (arr[front(&dq)] < min_of_max)
                min_of_max = arr[front(&dq)];
        }
