...
long long total = 0;
    ...
    for (int i = 0; i < petrolpumps_rows; i++) {
        long long ptrl = petrolpumps[i][0];
        long long distance = petrolpumps[i][1];
        long long gain = ptrl - distance;

        total += gain;
        current += gain;

        if (current < 0) {
            // Cannot start from previous start_index
            start_idx = i + 1;
            current = 0;
        }
    }

    // If total_balance is negative, no solution (but problem guarantees one)
