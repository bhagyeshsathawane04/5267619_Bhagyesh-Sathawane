...
char** bomberMan(int n, int grid_count, char** grid, int* result_count) {
    ...
    *result_count = grid_count;
    ...
    for (int i = 0; i < grid_count; i++) res[i] = malloc(c + 1);
            ...
            for (int j = 0; j < c; j++) {
                if (src[i][j] == 'O') {
                    ...
                    for (int k = 0; k < 4; k++) {
                        ...
                        if (ni >= 0 && ni < grid_count && nj >= 0 && nj < c)
                            res[ni][nj] = '.';
                    }
                }
            }
    ...
    if (n == 1) {
        for (int i = 0; i < grid_count; i++) strcpy(res[i], grid[i]);
        return res;
    }
    if (n % 2 == 0) {
        fill_all();
    ...
    char **first = malloc(grid_count * sizeof(char*));
    for (int i = 0; i < grid_count; i++) first[i] = malloc(c + 1);
    detonate(grid);
    for (int i = 0; i < grid_count; i++) strcpy(first[i], res[i]);
