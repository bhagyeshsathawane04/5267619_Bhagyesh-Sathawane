...
int unique = 0;
    ...
    // Allocate memory for the unique scores
    int* unique_r = (int*)malloc(unique * sizeof(int));
    if (ranked_count > 0) {
        unique_r[0] = ranked[0];
        int j = 1;
        for (int i = 1; i < ranked_count; i++) {
    ...
    *result_count = player_count;
    ...
    int ranked_index = unique - 1;
    
    // player's scores in ascending order
    for (int i = 0; i < player_count; i++) {
        // Find the player's rank using a two-pointer approach
        while (ranked_index >= 0 && player[i] >= unique_r[ranked_index]) {
            ranked_index--;
        }
        result[i] = ranked_index + 2;
    }
    
    // Free the dynamically allocated unique_ranked array
    free(unique_r);
