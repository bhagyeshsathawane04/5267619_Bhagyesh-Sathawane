 ...
 *
 */
 int compareBigIntegers(const void* a, const void* b) {
    char* s1 = *(char**)a;
    char* s2 = *(char**)b;
    ...
    if (len1 != len2)
        return len1 - len2; // shorter string = smaller number

    return strcmp(s1, s2);  // same length → lexicographical compare
}

char** bigSorting(int unsorted_count, char** unsorted, int* result_count) {
qsort(unsorted, unsorted_count, sizeof(char*), compareBigIntegers);
